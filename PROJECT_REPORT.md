...
- **Limbaj receptiv — „Identifică după categorie”:** Obiectiv dashboard **`receptive_identify_by_category`** (configurabil în `src/app/(dashboard)/receptive-language.tsx`: 2–3 categorii din `identifyByCategoryAssets`, 1–4 itemi per categorie). Rută **`objective=receptive_identify_by_category`** în `src/app/trial.tsx` (`selectedCategories` JSON, `itemsPerCategory`). Trial: **`src/features/receptive-language/ReceptiveIdentifyByCategoryTrial.tsx`** — 10 probe, grid `trialUiRootShellStyle`, imagini **`expo-image`**, progres `n / 10`, completare sesiune Firestore. Asset-uri: **`src/features/receptive-language/identifyByCategoryAssets.ts`** (generat cu **`node scripts/gen-receptive-identify-by-category-assets.js`**), foldere `assets/programe/limbaj-receptiv/identifica-dupa-categorie/imagini/{animale-domestice,animale-salbatice,fructe,legume,haine}/`, audio prompt per categorie în `.../audio/`, redare via **`playAudioModule`**. Logică: promptul urmează **categoria curentă**; după fiecare răspuns **corect** — sunet succes, apoi **prompt nou** (alegere pseudo-alternantă între categorii care mai au itemi neidentificați pe ecran); la **greșit** — **`playErrorAudio`** apoi **repetarea aceluiași prompt**. Itemii corecți **dispar de pe ecran** (`eliminatedKeys`) până la ecran gol, apoi trece la următoarea probă. **`objectiveTrialAvailability.ts`**: `RECEPTIVE_IMPLEMENTED_IDS` include `receptive_identify_by_category`.
- **Sortare — două obiective, același UI trial:** **`SortNonIdenticalTrial`** (`src/components/trials/SortNonIdenticalTrial.tsx`) cu `variant`: `"non_identical"` vs `"sort_by_category"`. **Sortare itemi non-identici** (`OBJECTIVES` id 2, `trialType: sort-non-identical-items`): categorii `caini`, `copaci`, `flori`, **`masini`**, `pasari`, `pesti`, `scaune`; imagini `assets/.../sortare-itemi-non-identici/imagini/<folder>/`. **`scripts/gen-sort-assets.js`** regenerează **`src/features/sort-non-identical/sortNonIdenticalAssets.ts`**; creează folder lipsă la nevoie. **Sortare pe categorie** (id 10, `sort-by-category`): `animale_domestice`, `animale_salbatice`, `fructe`, `legume`, `haine`; imagini `assets/.../sortare-pe-categorie/imagini/` (inclus **`caine.png`** în `animale-domestice`, în pool via `scripts/gen-sort-pe-categorie-assets.js` → `src/features/sort-pe-categorie/sortPeCategorieAssets.ts`). Generare asset: `node scripts/gen-sort-assets.js` / `node scripts/gen-sort-pe-categorie-assets.js`.
...
- **Obiective fără trial implementat:** Cardurile din grilele de obiective sunt **dezactivate vizual** (`opacity: 0.5` prin `src/design/objectiveCard.ts`) și **neclicabile** dacă nu există rută funcțională în `src/app/trial.tsx`. Logica centralizată: `src/utils/objectiveTrialAvailability.ts` (funcții per zonă: vizual, receptiv, etichetare, citire). Badge discret **„În curând”** pe cardurile dezactivate (unde e cazul). În `trial.tsx` există un comentariu care cere actualizarea acestui util la adăugare de ramuri noi.
  ...
  - **Limbaj receptiv:** Implementate **Arată obiecte comune** (`show_common_objects` → `objective=receptive_show_common_objects`) și **Identifică după categorie** (`receptive_identify_by_category`). Eliminat startul care ducea la `/trial` doar cu `sessionId` (trial B1 implicit incorect). **`canStart`** depinde de configurarea validă pentru obiectivul selectat.
...
- **`objective=receptive_identify_by_category`** → **`ReceptiveIdentifyByCategoryTrial`** (`sessionId`, `selectedCategories` JSON, `itemsPerCategory`, `voiceEnabled`).
...
- **`receptive-language.tsx`**, **`reading.tsx`**: Objective lists local to each file; configurable objectives use **`ItemSelector`** / setup drawer (receptiv: categorii pentru „Arată obiecte comune”; chip-uri 2–3 categorii + stepper itemi per categorie pentru „Identifică după categorie”). **Start** creează sesiune și navighează la **`/trial`** doar pentru obiective marcate ca implementate în **`objectiveTrialAvailability.ts`**. Cardurile fără trial sunt dezactivate (**„În curând”**). Receptiv: **`ReceptiveShowCommonObjectsTrial`** (categorii în **`categories.ts`**, asset-uri **`receptiveItemAssets.ts`**) și **`ReceptiveIdentifyByCategoryTrial`** (pool **`identifyByCategoryAssets.ts`**). Citire: **`receptive_letters`** folosește calea B1 cu parametri din ecran; celelalte obiective din listă sunt dezactivate până la implementare.
...
| `src/features/receptive-language/ReceptiveIdentifyByCategoryTrial.tsx` | Trial „Identifică după categorie”; prompturi per categorie, eliminare vizuală itemi corecți, sesiune 10 probe |
| `src/features/receptive-language/identifyByCategoryAssets.ts` | Pool imagini + mapare audio prompt per categorie (generat din `scripts/gen-receptive-identify-by-category-assets.js`) |
...
4. **Receptive / reading:** Obiectivele receptive fără ecran dedicat **nu mai pornesc** trial din dashboard (în afara **Arată obiecte comune** și **Identifică după categorie**). Pentru **citire**, doar **`receptive_letters`** este marcat implementat în helper — restul așteaptă implementare.
