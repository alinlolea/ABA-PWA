/**
 * Pattern Continuation Trial – "Continuare pattern"
 * Child sees a visible prefix of a repeated pattern and must complete the rest (left → right).
 */
...
export type PatternContinuationTrialState = {
  modelVisible: PatternItem[];
  expectedContinuation: PatternItem[];
...
function generateTrial(config: PatternTrialConfig): PatternContinuationTrialState {
  ...
  const fullSequence: PatternItem[] = [];
  ...
  const distractors = pickDistractorItems(fullSequence, numDistractors);
  const availableCubes = shuffle([...expectedContinuation, ...distractors]);

  return {
    modelVisible,
    expectedContinuation,
    placedCubes: Array(expectedContinuation.length).fill(null),
    availableCubes,
    placedPoolIndices: [],
    completed: false,
  };
...
export default function PatternContinuationTrial({
  sessionId,
  config,
  onComplete,
  voiceEnabled = true,
  ...
  const [session, setSession] = useState(() => ({
    trials: [] as PatternContinuationTrialState[],
    currentTrialIndex: 0,
    score: 0,
    completed: false,
  }));
...
type InnerProps = {
  trial: PatternContinuationTrialState;
...
function PatternContinuationTrialInner({
  trial,
  currentTrialIndex,
  voiceEnabled = true,
  screenWidth,
  ...
  const allPlaced = state.placedCubes.every((c, i) => itemsEqual(c, state.expectedContinuation[i]));
  ...
  const handleRelease = useCallback(
    (cubeIndex: number, item: PatternItem) =>
      (_: unknown, gestureState: { dx: number; dy: number }) => {
        ...
        const expected = state.expectedContinuation[nextExpectedIndex];
